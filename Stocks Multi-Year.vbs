...
' Lastly, Make the appropriate adjustments to your VBA script to enable it to run on every worksheet (that is, every year) at once.
    ...
    'Declare variables - we need two counters and the last row
    ...
    For Each ws In ThisWorkbook.Worksheets    '***Does not go past first ws***
    ws.Activate
    ...
    For i = 2 To LastRow
        ...
        If Cells(i, 1).Value <> Cells(i + 1, 1).Value Then 'Break if there is a change in ticker symbol
            ...
            Cells(SummaryRowNo, 10).Value = CDbl(Cells(i, 6).Value - YearOpenPrice) 'Enter yearly change on summary line- TO DO: SPECIFY 2 DPs
    ...
    MsgBox (SummaryRowNo) 'Delete before finalizing
    ...
    For i = 2 To (SummaryRowNo - 1)
        ...
        If Cells(i, 11).Value > GreatestIncrease Then
            ...
            'GreatestDecrease = Cells(i + 1, 11).Value
            'GreatestDecrTick = Cells(i + 1, 1).Value
        End If
...
Next ws 'Need to figure this out
